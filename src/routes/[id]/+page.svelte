<script lang="ts">
	...
	const POINT_LEGEND: Array<{ kind: 'endpoint' | 'observer' | 'node'; label: string; color: string }> = [
	...
	function connect() {
		...
		socket = new WebSocket(`/ws?browserId=anonymous`);
		socket.onopen = () => {
	...
	function totalPropagation(current: DiagnosticTest) {
	...
	function progressSteps(current: DiagnosticTest) {
		...
		const outboundAcks = ackObservations(outbound);
...
<main class="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-5 px-4 py-5 sm:px-6 lg:px-8">
	...
	{:else}
				...
				<div class="rounded-md border border-blue-200 bg-blue-50 px-3 py-2 transition">
					<div class="flex items-center gap-2">
						...
						<div>
							...
							<p class="font-semibold">{latency(firstPacketSeenAt(test), test.replyEndpointAckAt)}</p>
						</div>
					</div>
				</div>
				<div class="rounded-md border border-violet-200 bg-violet-50 px-3 py-2 transition">
					<div class="flex items-center gap-2">
						...
						<div>
							...
							<p class="font-semibold">{totalPropagation(test)}</p>
						</div>
					</div>
				</div>
			</div>
									...
									>
										...
										<td class="max-w-80 overflow-x-scroll no-scrollbar px-2 py-2">
											<div class="flex flex-wrap items-center gap-1">
												{#each compactPath(observation) as hop, index}
													{#if index > 0}
		...
		<section class="grid gap-3 md:grid-cols-4">
			...
			{@render Metric(t('metric.expires'), new Date(test.expiresAt).toLocaleTimeString(localeTag()))}
		</section>
	{/if}
</main>
	...
	<section class="rounded-md border border-neutral-300 bg-white p-4 shadow-sm">
		...
		<div class="mt-3 space-y-2 text-xs">
			<div class="flex flex-wrap items-center gap-2">
				<span class="font-semibold uppercase tracking-wide text-neutral-400">{t('map.legend.nodes')}</span>
				{#each POINT_LEGEND.filter((item) => mapPointKinds.has(item.kind)) as item (item.kind)}
					...
					>
						<span class="size-2 rounded-full" style={`background:${hidden ? '#d4d4d4' : item.color}`}></span>
			...
			{#if mapKinds.length}
				<div class="flex flex-wrap items-center gap-2">
					<span class="font-semibold uppercase tracking-wide text-neutral-400">{t('map.legend.links')}</span>
					{#each mapKinds as kind (kind)}
						...
						<button
							...
							title={hidden ? t('map.legend.show') : t('map.legend.hide')}
						>
							<span class="size-2 rounded-full" style={`background:${hidden ? '#d4d4d4' : kindColor(kind)}`}></span>
...
{#snippet InstructionsPanel(current: DiagnosticTest)}
	<section class="rounded-md border border-neutral-300 bg-white p-4 shadow-sm">
		...
		<div class="mt-3 rounded-md border border-neutral-200 bg-neutral-50 p-3">
			<p class="text-xs font-semibold uppercase tracking-wide text-neutral-500">{t('instr.tempCode')}</p>
		...
		<div class="mt-4 rounded-md border border-neutral-200 bg-neutral-50 p-3">
			<p class="text-xs font-semibold uppercase tracking-wide text-neutral-500">{t('instr.link')}</p>
			<div class="mt-2 flex items-start gap-2">
				<a
					class="mono min-w-0 flex-1 break-all text-xs text-teal-800 underline-offset-2 hover:underline"
					href={current.qrPayload}
				>
					{current.qrPayload}
				</a>
				<button
...
{#snippet RoutePanel(title: string, icon: string, observations: PacketObservation[])}
	...
	<section class="rounded-md border border-neutral-300 bg-white p-4 shadow-sm">
		...
		{#if observations.length}
			<div class="grid grid-cols-2 gap-2 text-sm xl:grid-cols-3">
				<div class="rounded-md bg-neutral-50 p-2">
					<p class="text-neutral-500">{t('route.routing')}</p>
					<p class="font-semibold">{routeKind(observations)}</p>
				</div>
				<div class="rounded-md bg-neutral-50 p-2">
					<p class="text-neutral-500">{t('route.hops')}</p>
					<p class="font-semibold">{bestHopCount(messages)}</p>
				</div>
				<div class="rounded-md bg-neutral-50 p-2">
					<p class="text-neutral-500">{t('route.deliveryTime')}</p>
					<p class="font-semibold">
			...
			<div class="mt-3 grid grid-cols-2 gap-2 text-sm xl:grid-cols-4">
				<div class="rounded-md bg-neutral-50 p-2">
					<p class="text-neutral-500">{t('route.observations')}</p>
					<p class="font-semibold">{messages.length}</p>
				</div>
				<div class="rounded-md bg-neutral-50 p-2">
					<p class="text-neutral-500">{t('route.observers')}</p>
					<p class="font-semibold">{observerCoverage(messages)}</p>
				</div>
				<div class="rounded-md bg-neutral-50 p-2">
					<p class="text-neutral-500">{t('route.paths')}</p>
					<p class="font-semibold">{uniquePathCount(messages)}</p>
				</div>
				<div class="rounded-md bg-neutral-50 p-2">
					<p class="text-neutral-500">{t('route.propagation')}</p>
					<p class="font-semibold">{propagationTime(messages)}</p>
				</div>
			</div>

			<div class="mt-3 rounded-md border border-neutral-200 bg-neutral-50 p-2 text-sm">
				<div class="flex flex-wrap items-center justify-between gap-2">
					<div>
						<p class="text-neutral-500">{ackLabel(acks, icon === 'send')}</p>
						<p class="font-semibold">
							{acks.length
